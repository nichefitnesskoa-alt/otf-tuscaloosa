...
-- Buddy Card: schema additions
ALTER TABLE public.leads
  ADD COLUMN IF NOT EXISTS referred_by_member_name text,
  ADD COLUMN IF NOT EXISTS referring_member_contact text,
  ADD COLUMN IF NOT EXISTS is_buddy_card boolean NOT NULL DEFAULT false;

ALTER TABLE public.intros_booked
  ADD COLUMN IF NOT EXISTS is_buddy_card_referral boolean NOT NULL DEFAULT false;

ALTER TABLE public.soml_pending_referrals
  ADD COLUMN IF NOT EXISTS discount_owed_to text,
  ADD COLUMN IF NOT EXISTS discount_owed_contact text,
  ADD COLUMN IF NOT EXISTS discount_owed_amount_cents integer,
  ADD COLUMN IF NOT EXISTS discount_honored_at timestamptz,
  ADD COLUMN IF NOT EXISTS discount_honored_by text;

-- Extend the pending-referral creator so buddy-card bookings also track the
-- $50-off credit owed to the referring member.
...
BEGIN
  ...
  -- Buddy Card: pull contact from the linked lead so admin knows who to credit
