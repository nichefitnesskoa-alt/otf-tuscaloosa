...
DECLARE
  ...
  v_is_buddy        boolean;
BEGIN
  -- Referring-member path: any explicit referral source OR a "(Friend)" variant
  -- with a captured referring member name. Single source of truth mirrored in
  -- src/lib/sa/leadsBooked.ts isReferralLikeSource().
  v_is_member_ref := (
      NEW.lead_source IN (
        'Member Referral',
  ...
  -- Paired friend-booking path (unchanged): friend booked at same time as
  -- referring member's intro, linked via paired_booking_id.
