...
export function MindbodyImportsPanel() {
  ...
  const handleCopy = async () => {
    ...
    const text = `Imported to Unified Portal — ${rangeLabel} (${visibleRows.length} total)\n\n${lines.join('\n')}`;
  ...
  const handleDownload = () => {
    ...
    a.href = url;
    a.download = `unified-portal-imports-${startYmd}_to_${endYmd}.csv`;
    a.click();
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Unified Portal Imports
          <Badge variant="secondary" className="ml-2">{visibleRows.length} people</Badge>
        </CardTitle>
        <p className="text-sm text-muted-foreground">
          Everyone an SA checked off as imported to Unified Portal. Pulls from both sourced leads and VIP rosters.
        </p>
      </CardHeader>
