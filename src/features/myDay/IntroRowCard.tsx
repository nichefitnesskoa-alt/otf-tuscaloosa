...
function TappableQBadge({ status, onTap, noQNeeded = false }: { status: UpcomingIntroItem['questionnaireStatus']; onTap: () => void; noQNeeded?: boolean }) {
  const [copied, setCopied] = useState(false);

  if (noQNeeded) return getQBadgeStatic(status, true);
...
export default function IntroRowCard({
  ...
  const isOutcomeOverdue = !item.latestRunResult && minutesUntilClass !== null && minutesUntilClass <= -120;
    ...
    <div className={cn("rounded-lg border bg-card overflow-hidden", isInFocusWindow && 'ring-2 ring-orange-500')}>
      ...
      {outcomeOpen && (
        <OutcomeDrawer
          ...
          initialIsWinback={item.latestRunIsWinback ?? false}
